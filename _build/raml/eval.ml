	...
	| Ref_swap, [l] -> begin
	    ...
	    let () = count (Mbfun_eval f) in
            store (Vbc Cunit)
				...
				| Prob_consume (qh, qt, qc), [l] -> (
					...
					| Vprob (a, b) -> begin
						...
						if kind = Efree then
	  					...
	  					costs := List.map2_exn !costs metrics (fun cost m ->
								if phys_equal m m_tick then
									cost ^- Float.(qh * p + qt * (1. - p) + qc * p * (1. - p))
								else
									cost ^- (m (Mbfun_eval f)));
						l
						end
				...
				| Prob_inv, [l] -> (
					match lookup l with
					| Vprob (a,b) -> count (Mbfun_eval f); store (Vprob (b,a))
					| _ -> raise wrong_arg_type
				)
				...
				| Prob_mult, [l] -> (
					...
					| Vtuple [l1;l2] -> (
						...
						| Vprob (a1,b1), Vprob (a2,b2) ->
							count (Mbfun_eval f); store (Vprob (a1*a2,a1*b2+b1*a2+b1*b2))
						| _ -> raise wrong_arg_type
					)
					| _ -> raise wrong_arg_type
				)

...
(* This is broken for lower bounds and constant time. The problem are
  ...
  let bf_fanno_type bf bf_type =
    match bf with
    | Prob_create ->
      assert (bf_type = Tarrow ([Ttuple [Tbase Tint; Tbase Tint]], Tprob, ()));
      let fanno =
        let fan_map fts =
          match fts with
          | [Ftuple [Fbase Tint; Fbase Tint]] ->
            let analyze rtanno metric =
              ...
              Anno.constant_tanno cost (Ttuple [Ttuple [Tbase Tint; Tbase Tint]]) rtanno
            in
            (Fprob, analyze)
          | _ ->
            raise (Analysis_error "Expecting function argument of type int * int.")
        in
          ...
          | [ftarg] ->
            let analyze rtanno metric =
              ...
              if not (phys_equal metric m_tick) then
                let gamma = Anno.amode_consume_prob_tanno (0.,0.,0.) rtanno (Ttuple [targ]) in
                Anno.add_cost_t gamma cost
              else
                let gamma = Anno.amode_consume_prob_tanno (qh,qt,qc) rtanno (Ttuple [targ]) in
                Anno.add_cost_t gamma cost
            in
            (Fprob, analyze)
          | _ -> raise (Analysis_error "Expecting function argument of type prob.")
        in
        { fan_targs = [targ]
        ; fan_tres = tres
        ; fan_map
      ...
      let fanno =
        let fan_map fts =
          ...
          | [Fprob] ->
            ...
            (Fprob, analyze)
          | _ ->
            raise (Analysis_error "Expecting function argument of type prob.")
        in
        { fan_targs = [Tprob]
        ; fan_tres = Tprob
        ; fan_map
  ...
  and forward_rec_bind
    ...
    let equalize_fanno_types ft1 ft2 =
      if compare_fanno_types ft1 ft2 then
        ...
        raise (Analysis_error ("Polymorphic recursion is currently not supported."
        ...
        let the_canno =
          ...
          | Eflip (e,e1,e2) ->
            ...
            let (gamma1,gamma2) =
              ...
              | _ -> raise (Analysis_error "Dead code.")
            in
            let gamma = Anno.combine_cannos_sym_prob ~on_var:(x,t) [gamma1; gamma2] in
            Anno.add_cost gamma (metric (Mflip))


          | Eflipc ((a,b),e1,e2) ->
            let gamma1 = backward { arg with anl_exp = e1; anl_level = arg_level + 1 } in
            let gamma2 = backward { arg with anl_exp = e2; anl_level = arg_level + 1 } in
            let (gamma1, gamma2) =
              match Anno.amode_patch_cannos [gamma1; gamma2] [[];[]] with
              | [g1; g2] -> (g1, g2)
              | _ -> raise (Analysis_error "Dead code.")
            in
            let gamma = Anno.combine_cannos_prob [float_of_int a /. float_of_int (a+b); float_of_int b /. float_of_int (a+b)] [gamma1; gamma2] in
            Anno.add_cost gamma (metric (Mflip))
